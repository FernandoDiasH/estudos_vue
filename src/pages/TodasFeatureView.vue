        ...
        <template #prototipo>
            <div class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <button class="border px-6 py-2 bg-slate-300 rounded-full" @click="toggle">Alternar</button>
                ...
                <div class=" flex flex-col gap-4">
                    ...
                    <form @submit.prevent="addTodo" class="flex gap-4">
                        <input class="border p-2 rounded-lg" v-model="newTodo" required placeholder="new todo">
                        <button>Add Todo</button>
                    </form>
                    <ul>
                        <li v-for="todo in todos" :key="todo.id" class="flex gap-4 justify-between p-2">
                            {{ todo.text }}
                            <button class="px-2 py-1 border bg-slate-200 rounded-lg" @click="removeTodo(todo)">X</button>
                        </li>
                    </ul>
                </div>
                ...
                <div class=" flex flex-col gap-4">
                    ...
                    <form @submit.prevent="addTodo2" class="flex gap-4">
                        <input class="border p-2 rounded-lg" v-model="newTodo2" required placeholder="new todo">
                        <button>Add Todo</button>
                    </form>
                    <ul>
                        <li v-for="todo in filteredTodos" :key="todo.id" class="flex gap-4 justify-between p-2">
                            <input type="checkbox" v-model="todo.done">
                            <span :class="{ done: todo.done }">{{ todo.text }}</span>
                            <button class="px-2 py-1 border bg-slate-200 rounded-lg" @click="removeTodo2(todo)">X</button>
                        </li>
                    </ul>
                    <div>
                        <button class="px-2 py-1 border bg-slate-200 rounded-lg" @click="hideCompleted = !hideCompleted">
                ...
                <div class=" flex flex-col gap-4">
                    <h2>Watchers</h2>
                    <p>Todo id: {{ todoId }}</p>
                    <button class="px-2 py-1 border bg-slate-200 rounded-lg" @click="todoId++" :disabled="!todoData">Próxima tarefa</button>
